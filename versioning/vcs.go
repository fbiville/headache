...
func GetLatestExecutionRevision(vcs Vcs) (string, error) {
	...
	trackingFile := fmt.Sprintf("%s/%s", root, ".headache-run")
	info, err := os.Stat(trackingFile)
	if os.IsNotExist(err) {
		err := ioutil.WriteFile(trackingFile, []byte("# Generated by headache -- commit me!"), 0640)
		return "", err
	}
	if info != nil && !info.Mode().IsRegular() {
		return "", fmt.Errorf("%s should be a regular file", trackingFile)
	}
	if err != nil {
		return "", err
	}

	sha, err := vcs.LatestRevision(trackingFile)
	if err != nil {
		return "", err
	}
	return sha, nil
